...
impl<'chars> Html5Parser<'chars> {
    ...
    fn do_parse(&mut self) -> Result<Vec<ParseError>> {
        ...
        let result = Ok(self.error_logger.borrow().get_errors());
        result
    }
    ...
    pub fn get_parse_errors(&self) -> Vec<ParseError> {
        self.error_logger.borrow().get_errors()
    }
    ...
    fn active_formatting_elements_clear_until_marker(&mut self) {
        while let Some(active_elem) = self.active_formatting_elements.pop() {
            if active_elem == ActiveElement::Marker {
                // Found the marker
                return;
            }
        }
    ...
    fn load_inline_stylesheet(&self, origin: CssOrigin, node: &Node) -> Option<CssStylesheet> {
        ...
        let source = match &self.document.get().location {
            Some(location) => format!("{location}#inline"),
    ...
    fn handle_link_element(&mut self, attributes: HashMap<String, String>) {
        ...
        if parser_in_body && !body_ok_types.contains(&rel.as_str()) {
            self.parse_error(
                format!(
                    "link element with rel attribute '{rel}' is not supported in the body"
                )
                .as_str(),
            );
            return;
        ...
        match rel.as_str() {
            ...
            _ => {
                self.parse_error(
                    format!("link element with rel attribute '{rel}' is not supported").as_str(),
                );
            }
        }
    }
